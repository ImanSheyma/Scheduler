...
public interface TaskRepository extends JpaRepository<Task, Integer>{
    ...
    @Query("SELECT t FROM Task t WHERE t.employee.id = :employeeId AND (t.start >= :start AND t.start <= :end OR t.end >= :start AND t.end <= :end OR t.start < :start AND t.end > :end)")
    List<Task> findTasksByDateRange(Integer employeeId, Date start, Date end);

    @Query("SELECT t FROM Task t WHERE (t.start >= :start AND t.start <= :end OR t.end >= :start AND t.end <= :end OR t.start < :start AND t.end > :end)")
